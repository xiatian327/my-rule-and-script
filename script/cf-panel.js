/*
 * Stash 测速面板脚本 (Tile 专用)
 * 供 CF-SpeedTest script-provider 使用
 */
...
function saveData(key, item) {
    if(!item) return;
    let data = { ip: item.ip, ping: Math.round(cleanNum(item.ping)), bw: Math.round(cleanNum(item.bandwidth)) };
...
$httpClient.get({ url: url, timeout: 15000 }, function(error, response, data) {
    let title = "Cloudflare 优选";
    let icon = "https://raw.githubusercontent.com/Koolson/Qure/master/IconSet/Color/Cloudflare.png";

    if (error) { 
        $done({ title: title, content: "网络错误: " + error, icon: icon }); 
        return; 
    }
    ...
    try {
        ...
        // 存入本地，供 cf-nodegen.js 生成节点使用
        saveData("CF_DATA_CM", cm);
        saveData("CF_DATA_CT", ct);
        saveData("CF_DATA_CU", cu);
        saveData("CF_DATA_V6", v6);
        
        // 构建 Stash 面板的展示内容 (Tile 显示)
        let panelContent = `📱 ${cm ? cm.ip : "无数据"}\n🌐 ${ct ? ct.ip : "无数据"}\n📶 ${cu ? cu.ip : "无数据"}\n🦕 ${v6 ? v6.ip : "无数据"}`;
        
        // Stash 中发送系统通知
        $notification.post("CF优选完成 ✅", "", "节点配置已自动更新，可测试连接");
        ...
        $done({ title: title, content: "脚本解析错误: " + e.message, icon: icon });
    }
});